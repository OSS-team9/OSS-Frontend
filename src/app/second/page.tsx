...
export default function MyPage() {
  ...
  // 4. ⭐️ (1단계 성공) Google 팝업 로그인을 성공했을 때
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      // 6. ⭐️ (2단계) 우리 백엔드 서버에 google_token 전송
      // (환경 변수에서 API 주소를 불러옴)
      ...
      // 7. ⭐️ (3단계) 우리 서버가 준 'access_token'을 전역 저장소에 저장!
  ...
  const fetchProtectedData = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/protected`, {
        method: 'GET',
        headers: {
          // 9. ⭐️ 헤더에 우리 서버가 준 토큰(JWT) 싣기
      ...
      // 10. ⭐️ API 명세서대로 `logged_in_as` 필드를 표시
